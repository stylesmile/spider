...
proxies = {
    'https': '183.154.52.85:9999',
    'https': '36.25.40.241:9999'
}
...
class NewsSpider:
    ...
    def parse_url_list(self, html):
        ...
        url_list = html.xpath("//div[@class='BlogcategoriesMobile']/select/option/@value")
        return url_list
    ...
    def get_url_list(self):  # 获取url列表
        try:
            category_html = self.parse_url(self.url_temp)
            category_url_list = self.parse_url_list(category_html)
            print(category_url_list)
            for url in category_url_list:
                detail_html = self.parse_url(self.url_temp_header + url)
                detail_url_list = self.parse_url_detail_list(detail_html)
                print(detail_url_list)
                for url2 in detail_url_list:
                    print(url2)
                    self.queue.put(self.host_header + url2)
    ...
    def save_content_list(self, content_list):  # 保存数据
        with open('尼泊尔语hamropatro文学博客.txt', 'a', encoding='utf-8') as f:
